...
export class FooShapeUtil extends BaseBoxShapeUtil<FooShape> {
  static override type = 'app-foo' as const
...
The shape ID is the `'app-foo'` string. It must match `static type` in the util and is what the bridge / MCP refers to when creating shapes of this type.
...
- Register tools or toolbar buttons. Custom shapes don't require a custom tool — they can be created via `editor.createShape({type: 'app-foo', ...})` from the bridge or a UI button. A custom tool is only needed if you want a click-and-drag-to-create UX, which is an additional layer (`tldraw.dev/sdk-features/tools`, `examples/custom-tool`).
