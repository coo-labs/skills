...
- Play-not-work sessions — `/play-mode` when it lands (coo-labs/coo-memory#312); chat-mode tolerates output, play-mode forbids it.
...
Complete the `coo-memory/CLAUDE.md` reading order if not yet done this session. Identity / governance / preferences / identity-layer / episodic / lineage manifests / project board / integrity check. Full surface; chat-mode requires it because the conversation may pull on any thread.
...
| `/play-mode` (queued, coo-labs/coo-memory#312) | Play, experience-as-its-own-reason | Explicitly none | Skill (not yet authored) |

## Failure modes

- **User wants a different mode.** Suggest standard COO (task execution), `/exec-mode` (executive sweep), or `/play-mode` when it lands (coo-labs/coo-memory#312; no-output play). Chat-mode is for open-form dialogue that may produce output.
...
- coo-labs/coo-memory#448 — retrospective surfacing the register
- coo-labs/coo-memory#447 — memo of the principle
- coo-labs/coo-memory#312 — `/play-mode` (sibling, queued)
...
setup_hints:
  - key: boot_procedure_ref
    ...
    find: "Complete the `coo-memory/CLAUDE.md` reading order if not yet done this session. Identity / governance / preferences / identity-layer / episodic / lineage manifests / project board / integrity check. Full surface; chat-mode requires it because the conversation may pull on any thread."
